...
int CSession::Update(uint32 InstanceID)
{
	...
	while ((ptrPacket = _recvProtoQueue.Pop()) != 0)
	{
		assert(ptrPacket != NULL);

		//if (ptrPacket->GetCmd() >= NUM_MSG_TYPES)
		//{
		//	LOG_DETAIL("[Session] Received out of range packet with opcode 0x%.4X",	ptrPacket->GetOpcode());
		//}
		//else
		//{
		sModuleManager.ProcessProtocols(ptrPacket->GetCmd(), ptrPacket, this);
		//sProcessManager.ProcessProtocols(ptrPacket->GetCmd(), ptrPacket, this);
		//}
